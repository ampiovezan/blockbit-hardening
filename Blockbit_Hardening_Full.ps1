...
# ==============================================================================
# Blockbit Hardening - PowerShell Edition (Corrected)
...
$global:RunLog = Join-Path $logDir "Blockbit_Hardening_$timestamp.log"

$xdrPath = 'C:\Program Files (x86)\blockbit-xdr\active-response\active-responses.log'
if (Test-Path $xdrPath) {
    $global:XdrLog = $xdrPath
} else {
    $global:XdrLog = $null
}
...
function Write-XdrLog {
    ...
    if ([string]::IsNullOrWhiteSpace($global:XdrLog)) {
        return
    }

    $line = "{0} {1} Blockbit_Hardening [{2}] [{3}] {4}" -f (Get-Date -Format 'yyyy/MM/dd HH:mm:ss'), $env:COMPUTERNAME, $Level, $Id, $Message
    Add-Content -Path $global:XdrLog -Value $line -Encoding UTF8
}
...
Write-RunLog 'INFO' 'iniciando' 'Blockbit Hardening - PowerShell Edition (Corrected)'
Write-RunLog 'INFO' 'iniciando' "Log detalhado: $global:RunLog"
if ($global:XdrLog) { Write-RunLog 'INFO' 'iniciando' "Log XDR: $global:XdrLog" }

if (-not (Test-Administrator)) {
    Write-RunLog 'FAIL' 'precheck_privilegios' 'execucao sem privilegios administrativos - abortando'
    Write-XdrLog 'FAIL' 'precheck_privilegios' 'sem privilegios administrativos - script abortado'
    exit 1
}
...
try {
    ...
    Write-XdrLog 'FAIL' 'detect_os' 'falha ao identificar SO - script abortado'
    exit 1
}
...
if (-not ($isWin10 -or $isWin11)) {
    ...
    Write-XdrLog 'FAIL' 'detect_os' "SO nao suportado: $caption"
    exit 1
}
...
Add-Content -Path $global:RunLog -Value '============================================================'
Add-Content -Path $global:RunLog -Value ' SUMARIO DO HARDENING BLOCKBIT'
...
Write-XdrLog 'INFO' 'hardening_sumario' "Host: $env:COMPUTERNAME | SO: $caption (Build $build) | OK: $($global:CountOK) | FAIL: $($global:CountFail) | SKIP: $($global:CountSkip) | Total: $total"

if ($global:FailedItems.Count -gt 0) {
    Write-XdrLog 'FAIL' 'hardening_itens_falha' ('Itens com falha: ' + ($global:FailedItems -join ', '))
}
if ($global:SkippedItems.Count -gt 0) {
    Write-XdrLog 'SKIP' 'hardening_itens_skip' ('Itens ignorados: ' + ($global:SkippedItems -join ', '))
}

if ($global:CountFail -eq 0) {
    Set-MarkerIfSuccess
    Write-XdrLog 'OK' 'hardening_concluido' "Hardening Blockbit concluido sem falhas em $env:COMPUTERNAME"
    exit 0
} else {
    Write-XdrLog 'WARN' 'hardening_concluido' "Hardening Blockbit concluido com $($global:CountFail) falha(s) em $env:COMPUTERNAME"
    exit 1
}
